...
class User{
    ...
    * @return array Returns list of users.
    */
    public function listMessageByUser($created_by){
        if(is_null($this->pdo)){
            $this->msg = 'Connection did not work out!';
            return [];
        }else{
            $pdo = $this->pdo;
            $sql = "SELECT m.id, m.message AS 'message', mp.message AS 'p_message', m.created_at AS 'created_at', CONCAT(u.fname,u.lname) AS 'user_name' FROM messages m LEFT JOIN messages mp ON mp.id = m.parent_id INNER JOIN users u ON u.id = m.created_by WHERE m.created_by = $created_by ";
    ...
    public function listMessages(){
        ...
        if(is_null($this->pdo)){
            ...
            return [];
             ...
             $data = array();
            $pdo = $this->pdo;
            $sql = "SELECT m.id,m.message AS 'message', m.parent_id as 'parent_id', m.created_at AS 'created_at',m.created_by AS 'created_by', CONCAT(u.fname,u.lname) AS 'user_name' FROM messages m INNER JOIN users u ON u.id = m.created_by WHERE m.parent_id < 1";
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
            $result = $stmt->fetchAll(); 
            ...
            foreach ($result as $key => $val) {
              ...
              $message_id = $val['id'];
            $sql2 = "SELECT  id, message,parent_id,created_at,created_by 
            from    (SELECT * from messages
                     order by parent_id, id) products_sorted,
                    (SELECT @pv := '$message_id' ) initialisation
            where   find_in_set(parent_id, @pv)
            and     length(@pv := concat(@pv, ',', id))";
            $stmt2 = $pdo->prepare($sql2);
            $stmt2->execute();
            $childrenData = array();
            $children = $stmt2->fetchAll();
            foreach ($children as $key2 => $val2) {
                $childrenData[$key2]['parent_message'] = $this->getReplyMessage($val2['parent_id']);
                $childrenData[$key2]['id'] = $val2['id'];
                $childrenData[$key2]['message'] = $val2['message'];
                $childrenData[$key2]['parent_id'] = $val2['parent_id'];
                $childrenData[$key2]['created_at'] = $val2['created_at'];
                $childrenData[$key2]['created_by'] = $val2['created_by'];
            }
            $data[$key]['child'] = $childrenData;       
            }
            return $data; 
        }
    }    
    ...
    * @return data.
    */
    public function getReplyMessage($p_id) {
            $pdo = $this->pdo;
            $sql = "SELECT message AS 'perant_message' FROM messages WHERE id = $p_id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
            $result = $stmt->fetchAll(); 
            return $result[0]['perant_message'];
    ...
    */
    ...
    /**
    * Template for messaging function
    * @return void.
    */
    ...
    */
    public function editMessage($id) {
        ...
        print_r($messages);
        print $this->render(editMessageForm,$messages);
    }


    /**
    * Template for messages function
    * @return void.
    */
    ...
    /**
    * Template for messaging form
    * @return void.
    */
    public function addMessage($p_id) {
        print $this->render(addMessageForm,$p_id);
    }

    /**
    * Register a new message  function
    * @param string $message User message.
    * @param string $parent_id User reference message id for reply.
    * @return boolean of success.
    */
    public function saveMessage($message,$parent_id){
        ...
        $stmt = $pdo->prepare('INSERT INTO messages (message, parent_id, created_by,created_at) VALUES (?, ?, ?, ?)');
        if($stmt->execute([$message,$parent_id,$_SESSION['user']['id'],$now])){
             return true;
        }else{
            $this->msg = 'Inesrting a new user failed.';
            return false;
